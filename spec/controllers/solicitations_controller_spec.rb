...
RSpec.describe SolicitationsController, type: :controller do
  ...
  describe 'Solicitation allocation in allocation period' do
    before(:each) do
      ...
      @school_room1 = SchoolRoom.create(name:"YY", vacancies: 50, discipline: @discipline1, course_ids: [@course1.id], coordinator_id: @coordinator1.id)
      @school_room2 = SchoolRoom.create(name:"YY", vacancies: 50, discipline: @discipline1, course_ids: [@course2.id], coordinator_id: @coordinator2.id)
      @period = Period.create(period_type:'Alocação', initial_date: Date.current - 5.days, final_date: Date.current + 5.days)
      sign_in(@user1)
    end
  ...
  describe 'Solicitation allocation in adjustment period' do
    before(:each) do
      ...
      @coordinator2 = Coordinator.create(user: @user2, course: @course2)
      @deg = Deg.create(user: @user3)
      @school_room1 = SchoolRoom.create(name:"YY", vacancies: 50, discipline: @discipline1, course_ids: [@course1.id, @course3.id], coordinator_id: @coordinator1.id)
      @school_room2 = SchoolRoom.create(name:"YY", vacancies: 50, discipline: @discipline2, course_ids: [@course2.id], coordinator_id: @coordinator2.id)
      ...
      @room = Room.create(code: 'S10', name: 'Superior 10', capacity: 50, computers: 50,
                          active: true, time_grid_id: 1, building: @building, details: "Nada informado", course: @course1)
  ...
  describe 'approve_solicitation' do
    before(:each) do
      ...
      @room_4 = Room.create(code: '987654', name: 'S8', capacity: 80, computers: 0, active: true, time_grid_id: 1, course: @course_2, building: @buildings[1], details: "Nada informado", category_ids: [@category.id])
      @discipline_4 = Discipline.create(code: '774', name: 'Artes Visuais', department: @department_3)
      @school_room_5 = SchoolRoom.create(name:'AA', discipline: @discipline_4, vacancies: 40, course_ids: [@course_4.id], coordinator_id: @coordinator_3.id)
